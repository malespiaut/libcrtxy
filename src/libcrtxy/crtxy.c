...
void blend(Uint8* dest_r, Uint8* dest_g, Uint8* dest_b, Uint8 src1_r, Uint8 src1_g, Uint8 src1_b, Uint8 src2_r, Uint8 src2_g, Uint8 src2_b, XY_fixed alpha, Uint16* gamma_s2l, Uint8* gamma_l2s);
...
void XY_draw_line_xiaolinwu(XY_fixed fsx1, XY_fixed fsy1, XY_fixed fsx2, XY_fixed fsy2, XY_color color, Uint16* gamma_s2l, Uint8* gamma_l2s);
void XY_draw_line_bresenham(XY_fixed fsx1, XY_fixed fsy1, XY_fixed fsx2, XY_fixed fsy2, XY_color);
...
void
XY_default_options(XY_options* opts)
{
  opts->displayw = 320;
  opts->displayh = 240;
  opts->displaybpp = 16;
...
int
XY_grab_envvar(char* v, int* i, int* o, char* s)
{
  if (getenv(v) != NULL)
  {
    s = getenv(v);
...
void
XY_complain_envvar(char* v, char* okay)
{
...
XY_bool
XY_parse_envvars(XY_options* opts)
{
  int nextint, nexton;
  char* nextstr = "";
...
int
XY_parse_options(int* argc, char* argv[], XY_options* opts)
{
  int i, j, eat, err_arg;
  int nextint;
  char* nextstr;
...
XY_bool
XY_load_options(XY_options* opts)
{
  XY_bool res;
  char* fname;
...
XY_bool
XY_load_options_from_file(char* fname, XY_options* opts, XY_bool ignore_unknowns)
{
  FILE* fi;
  char line[256];
  int i, err;
...
XY_init(XY_options* opts, XY_fixed canvasw, XY_fixed canvash)
{
  ...
  {
    XY_screen = SDL_SetVideoMode(opts->displayw, opts->displayh, opts->displaybpp, SDL_SWSURFACE);
  }
  else
  {
    XY_screen = SDL_SetVideoMode(opts->displayw, opts->displayh, opts->displaybpp, SDL_SWSURFACE | SDL_FULLSCREEN);
...
void
XY_print_options(FILE* fi, XY_options opts)
{
  fprintf(fi, "Screen size: %d x %d pixels\n", opts.displayw, opts.displayh);
  fprintf(fi, "Screen depth: %d bpp\n", opts.displaybpp);
  fprintf(fi, "Fullscreen: %s\n", (opts.fullscreen == XY_OPT_WINDOWED ? "no" : (opts.fullscreen == XY_OPT_FULLSCREEN_REQUEST ? "requested" : "required")));
  fprintf(fi, "Alpha-blending: %s\n", (opts.alpha == XY_OPT_ALPHA_BLEND ? "real" : (opts.alpha == XY_OPT_ALPHA_FAKE ? "fake" : "off")));
  ...
  fprintf(fi, "Scaling: %s\n", (opts.scaling == XY_OPT_SCALE_BEST ? "best" : "fast"));
}

XY_bitmap*
XY_load_bitmap(char* filename)
{
...
XY_bitmap*
XY_load_bitmap_from_buffer(unsigned char* buffer, int size)
{
  SDL_RWops* rw;
  SDL_Surface *surf, *dispsurf;
  XY_bitmap* xyb;
...
void
XY_free_bitmap(XY_bitmap* bitmap)
{
  if (bitmap != NULL)
  {
    if (bitmap->surf != NULL)
...
XY_bool
XY_set_background(XY_color color, XY_bitmap* bitmap, XY_fixed x, XY_fixed y, int posflags, int scaling)
{
  int w, h;
  int posx, posy;
...
SDL_Surface*
scale_surf_best(SDL_Surface* orig, int new_w, int new_h)
{
  /* FIXME: Implement it! */
...
SDL_Surface*
scale_surf_fast(SDL_Surface* orig, int new_w, int new_h)
{
  SDL_Surface* s;
  int x, y;
  Uint32 tr, tg, tb, ta;
  ...
  for (y = 0; y < new_h; y++)
  {
    for (x = 0; x < new_w; x++)
    {
      ...
      if (tmp != 0)
      {
        ...
        pp(s, x, y, SDL_MapRGB(s->format, (Uint8)tr, (Uint8)tg, (Uint8)tb), (XY_fixed)ta, NULL, NULL);
      }
    }
  }
...
XY_color
XY_setcolor(Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
  return ((r << 24) | (g << 16) | (b << 8) | a);
}

void
XY_getcolor(XY_color c, Uint8* r, Uint8* g, Uint8* b, Uint8* a)
{
  *r = (c >> 24) & 0xff;
  *g = (c >> 16) & 0xff;
  *b = (c >> 8) & 0xff;
...
void
XY_enable_background(XY_bool enable)
{
...
XY_start_frame(int fps)
{
  ...
  if (XY_background_change)
  {
    ...
    if (XY_background_bitmap != NULL && XY_background_bitmap_enabled == XY_TRUE)
      SDL_BlitSurface(XY_background_bitmap->surf, NULL, XY_screen, &XY_background_dest);
  }
  else
  {
    memcpy(XY_dirty_rects_erasure, XY_dirty_rects, sizeof(SDL_Rect) * XY_dirty_rect_count);
    XY_dirty_rect_erasure_count = XY_dirty_rect_count;

    for (i = 0; i < XY_dirty_rect_count; i++)
    {
      SDL_FillRect(XY_screen, &(XY_dirty_rects_erasure[i]), XY_background_color);
          ...
          XY_background_bitmap_enabled == XY_TRUE)
      {
        ...
        {
          SDL_BlitSurface(XY_background_bitmap->surf, &src, XY_screen, dest);
        }
      }
    }
  }
...
XY_lines*
XY_duplicate_lines(XY_lines* src)
{
  XY_lines* l;
...
XY_bool
XY_translate_lines(XY_lines* lines,
                   XY_fixed x,
                   XY_fixed y)
{
  int i;
...
XY_bool
XY_scale_lines(XY_lines* lines, XY_fixed xscale, XY_fixed yscale)
{
  int i;
...
XY_bool
XY_rotate_lines(XY_lines* lines, int angle)
{
  (void)lines;
  (void)angle;
  /* FIXME: Do it */
...
void
XY_draw_one_line(XY_line line)
{
  XY_draw_line(line.x1, line.y1, line.x2, line.y2, line.color, line.thickness);
}

void
XY_draw_line(XY_fixed x1, XY_fixed y1, XY_fixed x2, XY_fixed y2, XY_color color, XY_fixed thickness)
{
  int sx1, sy1, sx2, sy2;
  ...
  if (XY_antialias)
  {
    ...
    {
      XY_draw_line_xiaolinwu(fsx1, fsy1, fsx2, fsy2, color, XY_gamma_screen_to_linear_2_2, XY_gamma_linear_to_screen_2_2);
    }
    else
    {
...
void
XY_draw_line_xiaolinwu(XY_fixed fsx1, XY_fixed fsy1, XY_fixed fsx2, XY_fixed fsy2, XY_color color, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  ...
  if (abs(dx) > abs(dy))
  {
    ...
    putpixel(XY_screen, xpxl1, ypxl1, sdlcolor, XY_mult(brightness1, fixed_alph), gamma_s2l, gamma_l2s);
    putpixel(XY_screen, xpxl1, ypxl1 + 1, sdlcolor, XY_mult(brightness2, fixed_alph), gamma_s2l, gamma_l2s);
    ...
    putpixel(XY_screen, xpxl2, ypxl2, sdlcolor, XY_mult(brightness1, fixed_alph), gamma_s2l, gamma_l2s);
    putpixel(XY_screen, xpxl2, ypxl2 + 1, sdlcolor, XY_mult(brightness2, fixed_alph), gamma_s2l, gamma_l2s);
    ...
    for (x = xpxl1 + 1; x < xpxl2; x++)
    {
      ...
      putpixel(XY_screen, x, XY_ipart(intery) >> XY_FIXED_SHIFT, sdlcolor, XY_mult(brightness1, fixed_alph), gamma_s2l, gamma_l2s);
      putpixel(XY_screen, x, (XY_ipart(intery) >> XY_FIXED_SHIFT) + 1, sdlcolor, XY_mult(brightness2, fixed_alph), gamma_s2l, gamma_l2s);
  ...
  else
  {
    ...
    putpixel(XY_screen, xpxl1, ypxl1, sdlcolor, XY_mult(brightness1, fixed_alph), gamma_s2l, gamma_l2s);
    putpixel(XY_screen, xpxl1 + 1, ypxl1, sdlcolor, XY_mult(brightness2, fixed_alph), gamma_s2l, gamma_l2s);
    ...
    putpixel(XY_screen, xpxl2, ypxl2, sdlcolor, XY_mult(brightness1, fixed_alph), gamma_s2l, gamma_l2s);
    putpixel(XY_screen, xpxl2 + 1, ypxl2, sdlcolor, XY_mult(brightness2, fixed_alph), gamma_s2l, gamma_l2s);
    ...
    for (y = ypxl1 + 1; y < ypxl2; y++)
    {
      ...
      putpixel(XY_screen, XY_ipart(interx) >> XY_FIXED_SHIFT, y, sdlcolor, XY_mult(brightness1, fixed_alph), gamma_s2l, gamma_l2s);
      putpixel(XY_screen, (XY_ipart(interx) >> XY_FIXED_SHIFT) + 1, y, sdlcolor, XY_mult(brightness2, fixed_alph), gamma_s2l, gamma_l2s);
...
void
XY_draw_line_bresenham(XY_fixed fsx1, XY_fixed fsy1, XY_fixed fsx2, XY_fixed fsy2, XY_color color)
{
  Uint32 sdlcolor = XY_color_to_sdl_color(color);
...
void
XY_draw_point(XY_fixed x, XY_fixed y, XY_color color, XY_fixed thickness)
{
...
XY_fixed
XY_screenx_to_canvasx(int sx)
{
  return (sx * XY_canvasw) / XY_screen->w;
}

XY_fixed
XY_screeny_to_canvasy(int sy)
{
  return (sy * XY_canvash) / XY_screen->h;
}

void
XY_screen_to_canvas(int sx, int sy, XY_fixed* cx, XY_fixed* cy)
{
...
void
putpixel_16(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  (void)gamma_s2l;
  (void)gamma_l2s;
...
void
putpixel_24(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  (void)gamma_s2l;
  (void)gamma_l2s;
...
void
putpixel_32(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  (void)gamma_s2l;
  (void)gamma_l2s;
...
void
blend(Uint8* dest_r, Uint8* dest_g, Uint8* dest_b, Uint8 src1_r, Uint8 src1_g, Uint8 src1_b, Uint8 src2_r, Uint8 src2_g, Uint8 src2_b, XY_fixed alpha, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  XY_fixed antialpha;
...
void
putpixel_fakea_16(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  Uint8* p = (Uint8*)surface->pixels + y * surface->pitch + x * 2;
  Uint8 r1, g1, b1;
  Uint8 r, g, b;
  ...
  else if (alph < XY_FIXED_ONE)
  {
    ...
    blend(&r, &g, &b, r1, g1, b1, XY_background_r, XY_background_g, XY_background_b, alph, gamma_s2l, gamma_l2s);
...
void
putpixel_fakea_24(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  Uint8* p = (Uint8*)surface->pixels + y * surface->pitch + x * 3;
  Uint8 r1, g1, b1;
  Uint8 r, g, b;
...
void
putpixel_fakea_32(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  Uint8* p = (Uint8*)surface->pixels + y * surface->pitch + x * 4;
  Uint8 r1, g1, b1;
  Uint8 r, g, b;
...
void
putpixel_reala_16(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  Uint8* p = (Uint8*)surface->pixels + y * surface->pitch + x * 2;
  ...
  else if (XY_FIXED_ONE)
  {
    ...
    blend(&r, &g, &b, r1, g1, b1, r2, g2, b2, alph, gamma_s2l, gamma_l2s);
...
void
putpixel_reala_24(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  Uint8* p = (Uint8*)surface->pixels + y * surface->pitch + x * 3;
  Uint8 r1, g1, b1, r2, g2, b2;
  Uint32 oldpixel;
...
void
putpixel_reala_32(SDL_Surface* surface, int x, int y, Uint32 pixel, XY_fixed alph, Uint16* gamma_s2l, Uint8* gamma_l2s)
{
  Uint8* p = (Uint8*)surface->pixels + y * surface->pitch + x * 4;
  Uint8 r1, g1, b1, r2, g2, b2;
  Uint8 r, g, b;
...
/* Get a pixel: */
Uint32
getpixel_24(SDL_Surface* surface, int x, int y)
{
  Uint8* p;
  Uint32 pixel;
...
void
XY_add_dirty_rect(int x1, int y1, int x2, int y2)
{
  int tmp;
...
void
XY_merge_dirty_rects(SDL_Rect* rects, int* cnt)
{
  int i, j;
  XY_bool done;
...
XY_bool
XY_rects_intersect(SDL_Rect* rects, int r1, int r2)
{
...
void
XY_rects_combine(SDL_Rect* rects, int r1, int r2)
{
  int right1, bottom1;
  int right2, bottom2;
...
XY_bool
XY_lines_intersect(XY_line line1, XY_line line2, XY_fixed* intersect_x, XY_fixed* intersect_y, XY_intersection* result)
{
...
XY_bool
XY_line_groups_intersect(XY_lines* lines1, XY_lines* lines2)
{
  int i, j;
  ...
  for (i = 0; i < lines1->count; i++)
  {
    ...
    {
      if (XY_lines_intersect(lines1->lines[i], lines2->lines[j], NULL, NULL, NULL))
        return (XY_TRUE);
    }
  }
